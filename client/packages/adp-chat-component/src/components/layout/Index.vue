...
const upsertConversation = (conversation: ChatConversation) => {
    ...
    const idx = internalConversations.value.findIndex(item => item.Id === conversation.Id);
    if (idx >= 0) {
...
// 计算属性
const currentApplicationId = computed(() => actualCurrentApplication.value?.ApplicationId || props.currentApplicationId || '');
const currentApplicationAvatar = computed(() => actualCurrentApplication.value?.Avatar || '');
const currentApplicationName = computed(() => normalizeApplicationName(actualCurrentApplication.value?.Name));
...
const currentConversationTitle = computed(() => {
    ...
    const activeConversation =
        actualConversations.value.find(conversation => conversation.Id === conversationId) ||
...
const currentTermsStorageKey = computed(() => `${TERMS_ACCEPTED_STORAGE_PREFIX}:${currentTermsSectionKey.value}`);
