...
const assistantMessageAvatarAlt = computed(
  () =>
    ...
    (props.language?.startsWith("en") ? "Assistant" : "助手"),
);
...
const renderItems = computed<RenderItem[]>(() => {
  ...
  const hasToolCall = list.some(
    (m) => m.Type === "tool_call" || m.Type === "task_execution",
  );
...
const quoteInfos = computed<QuoteInfoLike[]>(() => {
  const source = useClawRender.value
    ? lastReplyMessage.value
    : primaryMessage.value;
  return collectFromMessageContents(
    source,
...
<template>
  ...
  <div
    ...
    :class="{
      'chat-message-row--assistant': !isFromSelf,
      'chat-message-row--user': isFromSelf,
    }"
  >
    <img
      v-if="!isFromSelf"
      ...
      <template #content>
        ...
        <div v-else>
          ...
          <div v-if="isFromSelf" class="user-message">
            ...
            <div
              v-if="imageAttachments.length > 0 && mode !== 'claw'"
            ...
            <div
              v-if="docAttachments.length > 0 && mode !== 'claw'"
          ...
          <div v-else-if="useClawRender" class="claw-render">
            <template
              v-for="(renderItem, rIdx) in renderItems"
              :key="'ri-' + rIdx"
            >
              <CollapsibleMessageGroup
                ...
                :isStreaming="
                  isLastCollapseStreaming && rIdx === renderItems.length - 1
                "
          ...
          <!-- assistant 图片附件 -->
          <div
            v-if="!isFromSelf && imageAttachments.length > 0 && mode === 'claw'"
            class="image-attachments"
          >
            <img
              v-for="(file, idx) in imageAttachments"
...
.references-container {
  margin: 0px var(--td-comp-margin-l) var(--td-comp-margin-xl)
    var(--td-comp-margin-l);
}
...
/* 1. Kill TDesign's outline variant bubble — we render our own */
.chat-message-row--assistant
  :deep(.t-chat__text--variant--outline .t-chat__detail),
.chat-message-row--user :deep(.t-chat__text--variant--outline .t-chat__detail) {
...
/* 2. Also zero out .t-chat__text padding since we control it */
.chat-message-row--assistant :deep(.t-chat__text),
.chat-message-row--user :deep(.t-chat__text) {
  padding: 0 !important;
}

/* ============================================
   STRIP TDESIGN DEFAULT BUBBLE STYLING
   ============================================ */

/* Assistant: outline variant puts border on .t-chat__detail */
.chat-message-row--assistant
  :deep(.t-chat__text--variant--outline .t-chat__detail) {
  border: none !important;
  border-radius: 0 !important;
...
/* User: base variant puts background on .t-chat__text */
.chat-message-row--user :deep(.t-chat__text--variant--base .t-chat__text) {
  background-color: transparent !important;
  border-radius: 0 !important;
}

/* Zero out .t-chat__text padding for both — we control spacing */
.chat-message-row--assistant :deep(.t-chat__text),
.chat-message-row--user :deep(.t-chat__text) {
  padding: 0 !important;
}
...
/* ============================================
   YOUR OWN BUBBLE STYLES (single source of truth)
   ============================================ */

/* Assistant bubble */
.chat-message-row--assistant :deep(.markdown-body.assistant) {
  background: var(--adp-chat-assistant-bubble-background, #fff);
}
...
.chat-message-row--user :deep(.markdown-body.user p) {
  color: inherit;
}
</style>
