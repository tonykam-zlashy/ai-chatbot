...
const panelParkStyle = computed(() => {
  ...
  if (actualIsOverlay.value) {
    const width =
      typeof actualWidth.value === "number"
        ? `${actualWidth.value}px`
        : actualWidth.value;
    const height =
      typeof actualHeight.value === "number"
        ? `${actualHeight.value}px`
        : actualHeight.value;
    return {
      width,
      height,
...
const handleToggleTheme = () => {
  ...
  if (props.onToggleTheme) {
    ...
    console.log(
      "[handleToggleTheme] switching from",
      internalTheme.value,
      "to",
      newTheme,
    );
...
const actualIsShowToggleButton = computed(
  () =>
    props.showToggleButton && props.chatbotConfig?.launcher.enabled !== false,
);
const actualLauncherIconUrl = computed(() => {
  if (
    props.chatbotConfig?.assistant.launcherAvatarUrl &&
    props.launcherIconUrl === DEFAULT_LAUNCHER_ICON
  ) {
...
const actualLauncherPrompt = computed(() => {
  ...
  if (props.chatbotConfig?.launcher.prompt !== undefined)
    return props.chatbotConfig.launcher.prompt;
...
const actualLauncherStyle = computed(() => {
  ...
  const position =
    props.launcherPosition === "bottom-right" && configLauncher
      ? configLauncher.position
      : props.launcherPosition || "bottom-right";
  const configuredOffsetX =
    props.launcherOffsetX === 18 && configLauncher
      ? configLauncher.offsetX
      : props.launcherOffsetX;
  const configuredOffsetY =
    props.launcherOffsetY === 18 && configLauncher
      ? configLauncher.offsetY
      : props.launcherOffsetY;
...
const configuredWidth = computed(
  () => props.chatbotConfig?.panel.width ?? props.width,
);
const configuredHeight = computed(
  () => props.chatbotConfig?.panel.height ?? props.height,
);
const actualWidth = computed(() =>
  actualIsOverlay.value
    ? normalizeWidth(configuredWidth.value)
    : configuredWidth.value,
);
const actualHeight = computed(() =>
  actualIsOverlay.value
    ? normalizeHeight(configuredHeight.value)
    : configuredHeight.value,
);
...
const actualAiWarningText = computed(() => {
  if (
    props.aiWarningText === DEFAULT_AI_WARNING_TEXT_ZH &&
    actualLanguage.value?.startsWith("en")
  ) {
...
const actualChatI18n = computed(
  () => props.chatI18n ?? defaultI18n.value.chatI18n,
);
const actualChatItemI18n = computed(
  () => props.chatItemI18n ?? defaultI18n.value.chatItemI18n,
);
const actualSenderI18n = computed(
  () => props.senderI18n ?? defaultI18n.value.senderI18n,
);
...
<template>
  <Teleport to="body">
    <div
      ...
      @click="setOpen(true)"
    >
      <span v-if="actualLauncherPrompt" class="toggle-btn__prompt">{{
        actualLauncherPrompt
      }}</span>
      <span
  ...
  <Teleport :to="actualContainer" :disabled="!actualIsOverlay">
    ...
    >
      <MainApp
        ...
        @selectApplication="
          (app: Application) => emit('selectApplication', app)
        "
        @selectConversation="
        ...
        @message="
          (code: MessageCode, message: string) =>
            emit('message', getMessage(code).type, message)
        "
        @conversationChange="
